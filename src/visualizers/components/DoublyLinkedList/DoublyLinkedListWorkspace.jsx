    ...
    return (
        <div className="dll-node-wrapper">
            <div className="dll-node-container">
                {!isHead && <div className="dll-connector-back"></div>}
                <div className={nodeClasses}>
                    <div className={`${pointerClasses} ${isHead ? 'is-head' : ''}`}></div>
                    <div className="ll-node-value">{value}</div>
                    <div className={`${pointerClasses} ${isTail ? 'is-tail' : ''}`}></div>
                </div>
                {!isTail && <div className="dll-connector-forward"></div>}
            </div>
        </div>
    );
};
...
function DoublyLinkedListWorkspace({ onBack }) {
    ...
    const handleAddAtIndex = () => {
        ...
        const result = dsDoublyLinkedList.current.addAtIndex(idx, val);
        if (typeof result === 'string') {showToast(result, 'error');}
        else if (result) {
            setAnimationHistory(result);
            setCurrentStep(0);
            setIsPlaying(true);
            setTimeout(() => setListState(dsDoublyLinkedList.current.getListState()), (result.length - 1) * animationSpeed);
        }
        setValue('');
        setIndex('');
    };
    ...
    const handleRemoveFromHead = () => {
        const result = dsDoublyLinkedList.current.removeFromHead();
        if (typeof result === 'string') {showToast(result, 'error');}
        else {handleOperation(() => result);}
    };

    const handleRemoveFromTail = () => {
        const result = dsDoublyLinkedList.current.removeFromTail();
        if (typeof result === 'string') {showToast(result, 'error');}
        else {handleOperation(() => result);}
    };
    ...
    const handleRemoveAtIndex = () => {
        ...
        const result = dsDoublyLinkedList.current.removeAtIndex(idx);
        if (typeof result === 'string') {showToast(result, 'error');}
        else if (result) {
            setAnimationHistory(result);
            setCurrentStep(0);
    ...
    const handleClear = () => {
        handleOperation(ds => ds.clear());
        setListState([]); 
    };

    const handleRandom = () => {
        const history = handleOperation(ds => ds.random());
        if (history) {
    ...
    return (
        <div className="ds-workspace">
            ...
            {isInfoPanelOpen && <InfoPanel data={dsInfo.doublyLinkedLists} onClose={() => setIsInfoPanelOpen(false)} />}
        </div>        
    );
}

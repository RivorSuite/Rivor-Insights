...
export class DS_SinglyLinkedList {
    ...
    addToHead(value) {
        const history = []; // This is a temporary array for building the animation states
    ...
    addToTail(value) {
        ...
        let current = this.head;
        let index = 0;
        while (current.next) {// Create a frame that says "highlight the current node"
    ...
    removeFromTail() {
        ...
        // Step 1: Highlight the second-to-last node
    ...
    removeAtIndex(index) {
        ...
        // separate, optimized removeFromTail is faster let it handle the last index case.
    ...
    random(minSize = 4, maxSize = 10, maxVal = 1000) {
        const history = []; // Start with a fresh history
        const recordHistory = (type, description, details = {}) => {
            history.push({
                type,
                description,
        ...
        for (let i = 0; i < size; i++) {
            ...
            const newNode = new Node(val, this.head); // For simplicity and speed add to the head each time
            this.head = newNode;
            this.size++;
        }

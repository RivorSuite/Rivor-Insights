...
export class DS_CircularLinkedList {
    ...
    addToHead(value) {
        const history = []; //Temporary array for building the animation states
    ...
    removeFromHead() {
        ...
        if (this.size === 0) {
            recordHistory('end', 'List is already empty.');
            return history;
        }
    ...
    removeAtIndex(index) {
        ...
        //A separate, optimized removeFromTail is faster let it handle the last index case.
    ...
    random(minSize = 4, maxSize = 10, maxVal = 1000) {
        ...
        this.head = null; //clear the existing list
        this.size = 0;

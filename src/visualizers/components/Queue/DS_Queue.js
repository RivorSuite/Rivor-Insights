...
export class DS_Queue {
    constructor(isFixedSize = false, capacity = 10) {
        this.queue = []; // store objects: { id, value }
        this.history = [];
    ...
    setCapacity(newCapacity) {
        if (this.isFixedSize) {
            ...
            if (this.queue.length > newCapacity) {this.queue.length = newCapacity;}
            this.history = [];
    ...
    enqueue(value) {
        ...
        this.queue.push({ id: itemId++, value: value });// Push an object with a unique ID

export class DS_Array {
    constructor() {
        this.array = [];
        this.history = []; // Store the "story" of the animation
    ...
    _recordHistory(type, description, details = {}) {
        const step = {
            ...
            ...details      // Extra info, { index: 2, value: 10 }
        };
        this.history.push(step);
    }

    
    insert(index, value) { // Inserts a value at a given index with a more detailed animation script
        if (index < 0 || index > this.array.length) {return `Index out of bounds. Next valid index is ${this.array.length}.`;}
        this.history = [];
    ...
    addToFront(value) {// Adds a value to the front of the array (index 0)
        // If the array is not empty, use the standard insert logic
        if (this.array.length > 0) {return this.insert(0, value);}
        this.history = [];
        this._recordHistory('start', `Adding ${value} to the front`);
        this.array.push(null); // Step 1: Add a placeholder and highlight it
        this._recordHistory('highlight', `Making space at index 0`, { indices: [0] });
        this.array[0] = value; // Step 2: Insert the new element into the spot
        this._recordHistory('insert', `Inserting ${value} at index 0`, { index: 0, value });
        this._recordHistory('end', `Addition to front complete`);
        return this.history;
    }
    addToBack(value) { // Adds a value to the back of the array
        ...
        this.array[index] = value; // Step 2: Insert the new element into the index spot 
        this._recordHistory('insert', `Inserting ${value} at index ${index}`, { index, value });
    ...
    random(minSize = 4, maxSize = 10, maxVal = 1000) { // Fills the array with random data within a specified size range
        this.history = [];
        this.array = [];
        this._recordHistory('start', `Generating a random array.`);
        const size = Math.floor(Math.random() * (maxSize - minSize + 1)) + minSize; //random sizing
        for (let i = 0; i < size; i++) {
            const val = Math.floor(Math.random() * maxVal); // Generate a random value (e.g., between 0 and 99)
            this.array.push(val);
        }
    ...
    removeAtIndex(index) { // Removes a value from a given index with animation script
        if (index < 0 || index >= this.array.length) {return `Index out of bounds: Cannot remove at index ${index}.`;}
        this.history = [];
    ...
    removeFromFront() { // Removes a value from the front of the array
        if (this.array.length === 0) {return "Cannot remove from an empty array.";}
        return this.removeAtIndex(0);
    }
    
    removeFromBack() { // Removes a value from the back of the array
        if (this.array.length === 0) { return "Cannot remove from an empty array.";}
    ...
    getSnapshot() { // A simple method to get the current state and history
        return {
            array: this.array,
